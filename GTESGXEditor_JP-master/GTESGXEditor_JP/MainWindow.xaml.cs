using GTESGXEditor_JP.Entities;
...
namespace GTESGXEditor_JP
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public byte[] ReadVAG(string path)
        {
            ...
            using (var stream = new BinaryStream(new MemoryStream(bytes)))
            {
                if (stream.ReadString(3) != "VAG")
                    throw new InvalidDataException("VAGファイルではありません。VAGファイルを開いて再試行してください。");
        ...
        private void RefreshSGXDEntries(bool fromListChange = false, bool skipEstimate = false)
        {
            ...
            if (lstEsgx != null)
            {
                if (!fromListChange)
                {
                    ...
                    if (!skipEstimate)
                    {
                        lblEstimate.Content = string.Format("ファイルサイズ: {0}KB", Math.Round(fileSize / 1000, 1));
                ...
                lblCurrentVag.Content = "使用するファイル:" + selectedSgxd.audioStreamName;
                if (selectedSgxd.audioStreamName == null)
                    lblCurrentVag.Content = "使用するファイル:デフォルト";
        ...
        private void UpdateSGXDEntry(int index)
        {
            ...
            {
                System.Windows.Forms.MessageBox.Show("入力形式が無効です。 値を入力して再試行してください。", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
            ...
            if (openFile.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        lblEstimate.Content = string.Format("ファイルサイズ: {0}KB", Math.Round(double.Parse(new FileInfo(openFile.FileName).Length.ToString()) / 1000, 1).ToString());
                        if (Math.Round(fileSize / 1000, 1) >= 200)
                        {
                            lblEstimate.Foreground = Brushes.Red;
                        }
        ...
        private void Save_Click(object sender, RoutedEventArgs e)
        {
            ...
            saveFile.Filter = "ESGXファイル・ESファイル|*.*";
        ...
        private void VAGExport_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (saveFile.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                {
                    int rpmFrequency_int = esgx.sampleSettings[i].rpmFrequency;
                    sgxd.WriteVAG(saveFile.SelectedPath, rpmFrequency_int);
                    i += 1;
                }
        ...
        {
            DialogResult result = System.Windows.Forms.MessageBox.Show("初期化してもよろしいですか？", "", MessageBoxButtons.YesNo, MessageBoxIcon.None, MessageBoxDefaultButton.Button2);
        ...
        private void btnDel_Click(object sender, RoutedEventArgs e)
        {
            if (lstEsgx.SelectedIndex != -1)
            {
                int returnindex = lstEsgx.SelectedIndex;
                esgx.sampleSettings.RemoveAt(lstEsgx.SelectedIndex);
                ...
                if (returnindex == lstEsgx.Items.Count)
                    lstEsgx.SelectedIndex = lstEsgx.Items.Count - 1;
                else
                    lstEsgx.SelectedIndex = returnindex;
